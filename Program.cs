...
async Task<(IPAddress ip, int? port, Dictionary<string,string> headers)> FindHueAsync()
{
...
async Task<int> Run(string[] args)
{
    ...
    var findHueTask = FindHueAsync();
    var timeOutTask = Task.Delay(5000);
    var result = await Task.WhenAny(new [] { findHueTask, timeOutTask});
    if(result == findHueTask)
    {
        var (hueip,port,headers) = await findHueTask;
        if(hueip==null) 
        {
